...
abstract class shopYmlupdateTransport implements Serializable
{
    const LOG_DEBUG = 5;
    const LOG_INFO = 4;
    const LOG_NOTICE = 3;
    ...
    /**
     * Get ymlupdate transport instance
     * @param string $id transport id
     * @param array $options
     * @param string $process_id
     * @throws waException
     * @return shopYmlupdateTransport
     */
    public static function getTransport($id, $options = array(), $process_id = null)
    {
        $class = 'shopYmlupdate'.ucfirst($id).'Transport';
    ...
    protected function log($message, $level = self::LOG_WARNING, $data = null)
    {
        if ($level <= $this->getOption('debug', self::LOG_WARNING)) {
            ...
            waLog::log($this->process_id.': '.$this->getLogLevelName($level).': '.$message, 'shop/plugins/ymlupdate.log');
        }
    }
    ...
    {
        if (!$this->temp_path) {
            $this->temp_path = wa()->getTempPath('plugins/ymlupdate/'.$this->process_id.'/', 'shop');
    ...
    {
        return 'shop.ymlupdate.'.preg_replace('@(^shopYmlupdate|Transport$)@', '', get_class($this));
    }
...
$(':input[name="{$feature_name}"]:first').unbind('change.ymlupdate').bind('change.ymlupdate',function(){
    var input = $(this);
...
HTML;
    ...
    public static function enumerate()
    {
        ...
        foreach (waFiles::listdir($dir) as $file) {
            if (($file != __FILE__) && preg_match('@^shopYmlupdate(\w+)Transport\.class\.php@', $file, $matches)) {
